<script lang="ts">
	...
	import { getCheckinStatus, doCheckin } from '$lib/apis/credit';
	...
	let checkinStatus = { checked_in: false, enabled: false, reward: 0 };
	let isCheckingIn = false;

	const loadCheckinStatus = async () => {
		try {
			const res = await getCheckinStatus(localStorage.token);
			if (res) {
				checkinStatus = res;
			}
		} catch (e) {
			console.error('Error fetching checkin status:', e);
		}
	};
	...
	const handleCheckin = async () => {
		isCheckingIn = true;
		try {
			const res = await doCheckin(localStorage.token);
			if (res) {
				toast.success($i18n.t('Check-in successful! +{{reward}} credit', { reward: res.reward }));
				checkinStatus = { checked_in: true, enabled: true, reward: res.reward };
			}
		} catch (e) {
			toast.error(`${e}`);
		} finally {
	...
	<slot name="content">
		...
		>
			...
			{#if checkinStatus.enabled}
				<div class="mx-1 mb-1">
					<button
						class="w-full px-3 py-1.5 gap-1.5 rounded-xl transition text-xs flex items-center justify-center {checkinStatus.checked_in ? 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 cursor-default' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/40'}"
						type="button"
						disabled={checkinStatus.checked_in || isCheckingIn}
						on:click={handleCheckin}
					>
						{#if checkinStatus.checked_in}
							<span>✅</span>
							<span>{$i18n.t('Checked in today')}</span>
						{:else if isCheckingIn}
							<span>⏳</span>
							<span>{$i18n.t('Checking in...')}</span>
						{:else}
							<span>🎁</span>
							<span>{$i18n.t('Daily Check-in')}</span>
						{/if}
					</button>
				</div>
			{/if}
