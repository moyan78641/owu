...
export const getCheckinStatus = async (token: string) => {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/credit/checkin/status`, {
		method: 'GET',
		headers: {
			Authorization: `Bearer ${token}`
		}
...
export const doCheckin = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/credit/checkin`, {
		method: 'POST',
		headers: {
			Authorization: `Bearer ${token}`,
			'Content-Type': 'application/json'
