...
class UsageConfigForm(BaseModel):
    ...
    SPARKLOC_PAYMENT_TOKEN: Optional[str] = None
    SPARKLOC_PAYMENT_ID: Optional[str] = None
    SPARKLOC_CALLBACK_HOST: Optional[str] = None
    CHECKIN_ENABLED: bool = Field(default=True)
    CHECKIN_MIN_CREDIT: float = Field(default=1, ge=0)
    CHECKIN_MAX_CREDIT: float = Field(default=5, ge=0)
...
async def get_usage_config(request: Request, _=Depends(get_admin_user)):
    return {
        ...
        "SPARKLOC_PAYMENT_TOKEN": request.app.state.config.SPARKLOC_PAYMENT_TOKEN,
        "SPARKLOC_PAYMENT_ID": request.app.state.config.SPARKLOC_PAYMENT_ID,
        "SPARKLOC_CALLBACK_HOST": request.app.state.config.SPARKLOC_CALLBACK_HOST,
        "CHECKIN_ENABLED": request.app.state.config.CHECKIN_ENABLED,
        "CHECKIN_MIN_CREDIT": request.app.state.config.CHECKIN_MIN_CREDIT,
        "CHECKIN_MAX_CREDIT": request.app.state.config.CHECKIN_MAX_CREDIT,
    }
...
async def set_usage_config(
    ...
    request.app.state.config.SPARKLOC_PAYMENT_TOKEN = form_data.SPARKLOC_PAYMENT_TOKEN
    request.app.state.config.SPARKLOC_PAYMENT_ID = form_data.SPARKLOC_PAYMENT_ID
    request.app.state.config.SPARKLOC_CALLBACK_HOST = form_data.SPARKLOC_CALLBACK_HOST
    request.app.state.config.CHECKIN_ENABLED = form_data.CHECKIN_ENABLED
    request.app.state.config.CHECKIN_MIN_CREDIT = form_data.CHECKIN_MIN_CREDIT
    request.app.state.config.CHECKIN_MAX_CREDIT = form_data.CHECKIN_MAX_CREDIT
