...
from open_webui.config import EZFP_CALLBACK_HOST, ALIPAY_APP_ID, SPARKLOC_PAYMENT_TOKEN
...
from open_webui.utils.credit.sparkloc import sparkloc_client
...
async def create_ticket(
    ...
    elif form_data["pay_type"] == "sparkloc" and SPARKLOC_PAYMENT_TOKEN.value:
        detail = await sparkloc_client.create_trade(
            out_trade_no=out_trade_no, amount=form_data["amount"]
        )
    else:
...
@router.get("/callback/sparkloc", response_class=RedirectResponse)
async def sparkloc_callback(request: Request) -> RedirectResponse:
    callback = dict(request.query_params)
    log.info("sparkloc callback: %s", json.dumps(callback))
    if not sparkloc_client.verify(callback):
        log.warning("sparkloc callback invalid signature")
    ...
    # find ticket
    out_trade_no = callback.get("external_reference", "")
...
async def get_checkin_status(
    ...
    if not enabled:
        return {"checked_in": False, "enabled": False, "reward": 0}
    ...
    for log_item in logs:
        if (
            ...
            and log_item.detail
            and log_item.detail.desc == "daily_checkin"
        ):
            return {
                "checked_in": True,
                "enabled": True,
    ...
    return {"checked_in": False, "enabled": True, "reward": 0}
...
async def do_checkin(request: Request, user: UserModel = Depends(get_verified_user)):
    ...
    enabled = request.app.state.config.CHECKIN_ENABLED
    if not enabled:
        raise HTTPException(status_code=400, detail="Check-in is disabled")
    ...
    min_credit = float(request.app.state.config.CHECKIN_MIN_CREDIT)
    max_credit = float(request.app.state.config.CHECKIN_MAX_CREDIT)
    reward = round(random.uniform(min_credit, max_credit), 2)
    ...
    Credits.add_credit_by_user_id(
        AddCreditForm(
            ...
            detail=SetCreditFormDetail(desc="daily_checkin"),
        )
    )
